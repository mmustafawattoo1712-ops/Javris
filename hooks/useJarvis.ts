...
export const useJarvis = () => {
  ...
  const executeAppCommand = useCallback((appName: string, actionType: string, payload: string = '') => {
      ...
      if (actionType === 'close') {
          setActiveApp(null); 
          // @ts-ignore
          if (typeof window.JarvisBridge !== 'undefined') window.JarvisBridge.performAction('HOME'); 
          return; 
      }
      setActiveApp(appName); 
      // @ts-ignore
      if (typeof window.JarvisBridge !== 'undefined') window.JarvisBridge.launchApp(packageName, actionType, payload);
  }, []);
  ...
  const toggleSystemSetting = useCallback((setting: string, state?: boolean) => {
      setDeviceState(prev => {
          ...
          // @ts-ignore
          if (typeof window.JarvisBridge !== 'undefined') window.JarvisBridge.toggleSystemSetting(setting, newState);
                ...
                callbacks: {
                    ...
                    onmessage: async (msg: LiveServerMessage) => {
                        ...
                        if (msg.serverContent?.turnComplete) {
                           // Handle chat log updates if needed
                        }
                        ...
                        if (msg.toolCall) {
                            for (const fc of msg.toolCall.functionCalls) {
                                ...
                                if (fc.name === 'control_installed_app') {
                                    ...
                                    // @ts-ignore
                                    if(window.JarvisBridge) window.JarvisBridge.sendWhatsAppMessage(fc.args['contact_name'], fc.args['message_body']);
                                    result.info = "Message sent.";
                                } else if (fc.name === 'make_phone_call') {
                                    // @ts-ignore
                                    if(window.JarvisBridge) window.JarvisBridge.makePhoneCall(fc.args['contact_name']);
                                    ...
                                    // @ts-ignore
                                    toggleSystemSetting(fc.args['target'], fc.args['state'] === 'on');
                                    ...
                                    if (fc.args['pin_code'] === SECURITY_PIN) {
                                        // @ts-ignore
                                        if(window.JarvisBridge?.unlockDevice) window.JarvisBridge.unlockDevice();
                                        unlockSystem();
